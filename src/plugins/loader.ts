...
function listPluginSdkSubpathsFromFs(modulePath: string): string[] {
  const candidates = new Set<string>();
  let cursor = path.dirname(modulePath);
  for (let i = 0; i < 6; i += 1) {
    for (const relativeDir of ["src/plugin-sdk", "dist/plugin-sdk"]) {
      const dir = path.join(cursor, relativeDir);
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  ...
  if (!packageRoot) {
    return listPluginSdkSubpathsFromFs(modulePath);
  }
  ...
  try {
    ...
    // Fall back to scanning plugin-sdk files when package metadata is unavailable.
  }
  return listPluginSdkSubpathsFromFs(modulePath);
}
