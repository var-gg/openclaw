...
function shouldFallbackToCopyOnWindows(error) {
...
function symlinkPath(sourcePath, targetPath, type) {
  try {
    ...
    // Windows developer mode / symlink policy is inconsistent across local
    // hosts. For regular runtime assets, copying preserves runtime semantics
    // and keeps local builds working without weakening directory junction use.
    if (!shouldFallbackToCopyOnWindows(error)) {
      throw error;
    }
    removePathIfExists(targetPath);
